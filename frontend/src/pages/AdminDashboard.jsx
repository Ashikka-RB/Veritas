...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [resMetrics, resWeekly, resFraud, resQueue] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL}/api/admin/analytics/metrics`, { headers }),
        fetch(`${import.meta.env.VITE_API_URL}/api/admin/analytics/weekly-verifications`, { headers }),
        fetch(`${import.meta.env.VITE_API_URL}/api/admin/analytics/fraud-distribution`, { headers }),
        fetch(`${import.meta.env.VITE_API_URL}/api/admin/review-queue`, { headers })
      ]);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/approve/${id}`, {
        method: 'POST',
        headers: { 
  ...
  const handleReject = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/reject/${id}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleFlagFraud = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/flag-fraud/${id}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
